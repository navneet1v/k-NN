#!/usr/bin/env python3
"""SIFT-1M benchmark for ClusterANN IVF on OpenSearch k-NN."""
import struct
import numpy as np
import requests
import time
...
INDEX = "clusterann-sift-1m"
FIELD = "vector"
DIM = 128
BATCH_SIZE = 5000

def read_sift_bin(filename):
...
def bulk_index(base):
    ...
    for i in range(0, len(base), BATCH_SIZE):
        ...
        if resp.status_code != 200:
            ...
            sys.exit(1)
        errors = resp.json().get('errors', False)
        if errors:
...
if __name__ == '__main__':
    ...
    base = read_sift_bin('/Users/viktari/pysptag/data/sift/base.bin')
    queries = read_sift_bin('/Users/viktari/pysptag/data/sift/query.bin')
    groundtruth = read_groundtruth('/Users/viktari/pysptag/data/sift/groundtruth.bin')
    print(f"Base: {base.shape}, Queries: {queries.shape}, GT: {groundtruth.shape}")

    # Delete old index
    requests.delete(f"{HOST}/{INDEX}")

    # Create index with ClusterANN method
    mapping = {
        "settings": {
            "index": {
                "knn": True,
