...
final class LeafHits {
     ...
     * — and knowing its own floor is what lets its scorer prune.
     */
     ...
     *
     * <p><b>Bounded, and it tells the scorer its bound.</b> A segment cannot contribute more than {@code topN} to a
     * global top-{@code topN}, so keeping more is waste — but the reason to keep a heap rather than a list is what
     * happens once it is full: the floor is pushed back into the scorer, and a scorer that scores in batches then skips
     * whole batches whose best is already below it, without reading a vector in them.
     *
     * <p>That is the same feedback a Lucene collector would provide, and without it a stage that feeds another stage
     * loses the pruning that the same stage enjoys when it feeds the caller instead — which is the configuration that can
