 ...
 *
 * <p>{@link #isOptimistic()} is left at its default of {@code false}, deliberately. Lucene's optimistic mode runs a first
 * pass with a reduced per-leaf k and then re-enters the segments whose results reached the global bar. That is a good
 * trade for a graph, where the cost is nodes visited; a cluster scan's cost is the postings it reads, and re-entering a
 * segment means reading them again. So the second pass is not switched off here — it is never asked for.
 */
