...
public class ExactSearcher {
    ...
    public Scorer exactSearchScorer(final LeafReaderContext leafReaderContext, final ExactSearcherContext context) throws IOException {
        ...
        if (context.getRadius() != null) {
            assert FieldInfoExtractor.extractKNNEngine(fieldInfo) == KNNEngine.FAISS : "Exact searcher for Radial search is only used by FAISS engine";
            final float minScore = context.isMemoryOptimizedSearchEnabled
                ? context.getRadius()
                : KNNEngine.FAISS.score(context.getRadius(), FieldInfoExtractor.getSpaceType(modelDao, fieldInfo));
