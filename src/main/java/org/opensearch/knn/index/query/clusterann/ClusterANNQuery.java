 ...
 *
 * <p><b>Nothing is searched until a scorer is asked for.</b> Two reasons. OpenSearch rewrites a query again during the
 * fetch phase, so work done in {@link #rewrite} is paid for twice — hence {@code rewrite} returns {@code this}. And a
 * rescorer already fans out per segment, so searching every segment in {@link #createWeight} would mean two fan-outs
 * and an intermediate merge the rescorer immediately takes apart.
 *
