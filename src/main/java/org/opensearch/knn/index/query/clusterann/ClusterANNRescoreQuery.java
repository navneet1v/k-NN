 ...
 * </pre>
 *
 * <p><b>The exact pass hands back a scorer rather than results.</b> Lucene's collector does the top-k, so nothing here
 ...
 *
 * <p>The exact scoring itself is {@link ExactSearcher}'s, so the vector reads and the nested best-child handling are the
 * ones already written rather than a second copy.
 */
@Log4j2
public class ClusterANNRescoreQuery extends Query {
    ...
    /**
     * Resolved when a weight is created, not when the query is built.
     *
     * <p>Building the searcher means reaching for the {@link ModelDao} singleton, and a query object can be built and
     * never executed — rewritten away, or constructed to be compared against another. Constructing it here would make
     * that a call into plugin state that the query may have no use for.
     */
    private final Supplier<ExactSearcher> exactSearcher;
    ...
    ClusterANNRescoreQuery(
        ...
        final float[] queryVector,
        @Nullable final BitSetProducer parentsFilter,
        final Supplier<ExactSearcher> exactSearcher
    ) {
        if (firstPassK < k) {
