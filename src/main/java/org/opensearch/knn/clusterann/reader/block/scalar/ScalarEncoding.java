...
/**
 * How a scalar quantized vector is encoded: the code width per dimension on the document side, the width the
 * query is quantized to, and the packing that follows from the two.
 *
 * <p>Copied verbatim (minus the enclosing format class) from Lucene 10.4's
 * {@code Lucene104ScalarQuantizedVectorsFormat.ScalarEncoding}. OpenSearch 3.5 ships Lucene 10.3, which does
 * not carry the enum, and the ClusterANN read path needs the packing arithmetic to locate codes within a
 * block. Delete this class and use Lucene's once the bundled Lucene has it.
 */
public enum ScalarEncoding {
    ...
    public int getDiscreteDimensions(int dimensions) {
        ...
        int maxDiscretized = Math.max(queryDiscretized, docDiscretized);
        assert maxDiscretized % (8.0 / queryBitsPerDim) == 0 : "bad discretized=" + maxDiscretized + " for dim=" + dimensions;
        assert maxDiscretized % (8.0 / bitsPerDim) == 0 : "bad discretized=" + maxDiscretized + " for dim=" + dimensions;
        return maxDiscretized;
    }
